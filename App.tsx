...
import { UserIdentity, GeoLocation } from './types';
import { getCurrentPosition, getH3Index, getNeighbors, formatH3 } from './utils/location';
import { generateIdentity } from './utils/crypto';
import { useGeoSwarm } from './hooks/useGeoSwarm';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        setError("Location permission required for P2P mesh.");
      }
    };
    init();
  }, []);
  ...
  if (error) return <div className="h-screen flex items-center justify-center p-8 text-center">{error}</div>;
  if (!location || !identity) return <div className="h-screen flex items-center justify-center bg-slate-900 text-emerald-500 font-black animate-pulse">BOOTING SECURE MESH...</div>;
  ...
  return (
    <div className="flex flex-col h-[100dvh] max-w-md mx-auto bg-slate-50 overflow-hidden shadow-2xl relative">
      <header className="bg-white border-b border-slate-100 p-4 z-30 shrink-0 flex items-center justify-between">
        ...
        <button onClick={() => setShowHelp(true)} className="p-2 text-slate-400"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
      </header>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6" onClick={() => setShowHelp(false)}>
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <ul className="space-y-3 text-sm text-slate-600">
              ...
              <li className="flex gap-2"><span>🕵️</span> <strong>K-Anonymity:</strong> Your precise location is masked by a ~1.2km hexagonal radius.</li>
            </ul>
            <button onClick={() => setShowHelp(false)} className="w-full py-3 bg-slate-900 text-white rounded-2xl font-bold">Close</button>
          </div>
        </div>
      )}
